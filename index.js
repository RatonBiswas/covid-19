...
function getCountries() {
  return new Promise((resolve, reject) => {
    fetch(`${BASE_URL}/countries`)
      ...
      .catch(err => {
        reject(err);
        errorDiv.innerText = "Can't found country list";
      });
  });
}
  ...
  const countryStats = `
        <div class="row">
            <div class="col-sm-4">
                <div class="card">
                <div class="card-body">
                    <h5 class="card-title">Total Confirmed: ${stats.confirmed.value}</h5>
                </div>
                </div>
            </div>
            <div class="col-sm-4">
                <div class="card">
                <div class="card-body">
                    <h5 class="card-title">Total Recovered: ${stats.recovered.value}</h5>
                </div>
                </div>
            </div>
             <div class="col-sm-4">
                <div class="card">
                <div class="card-body">
                    <h5 class="card-title">Total Deaths: ${stats.deaths.value}</h5>
                </div>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-sm-12 mt-4">Last Updated on ${niceDate}</div>
        </div>
    `;
...
function getLastDataUpdateDate(lastUpdate) {
  return `${makeToDigits(lastUpdate.getDate())}/${makeToDigits(
    ...
    lastUpdate.getHours()
  )}:${makeToDigits(lastUpdate.getMinutes())} min`;
}
...
function getStatistics(countryCode) {
  ...
  return new Promise((resolve, reject) => {
    fetch(url)
      ...
      .catch(err => {
        reject(err);
        errorDiv.innerText = `Can't found country list ${countryCode}`;
      });
  });
}
